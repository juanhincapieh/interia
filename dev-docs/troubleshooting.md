...
| `Connect request rejected` + Intelligence `VALIDATION_ERROR` (400) on `/agent/.../connect` | Seeded users used the wrong `organization_id` (older seed defaulted to `casa-de-erlang` while compose uses `hackathon`) | Run `npm run seed` again (script updates `organization_id` on conflict), then hard-refresh the app. |
| BFF: `Failed to fetch tools from MCP` / `ECONNREFUSED` (often port 3001) | `MCP_SERVER_URL` or BFF default doesn’t match `apps/mcp` (**3011**) or MCP isn’t running | Run `npm run dev:full` or `npm run dev:mcp`; set `MCP_SERVER_URL=http://localhost:3011/mcp` in `.env`. |
| BFF: `[LangGraph] runAgentStream error: Unexpected end of JSON input` | BFF pointed at the wrong LangGraph port (**8123** vs `dev:agent` **8133**) or agent not running | Set `LANGGRAPH_DEPLOYMENT_URL=http://localhost:8133`, restart the BFF, confirm the agent log binds to `:8133`. |
| Agent log: `ValueError: contents are required` (``google.genai`` / Gemini) with `AGENT_RUNTIME=gemini-flash-deep` | Deep Agents summarization middleware occasionally trims the chat to zero user/model turns before a Gemini call | Fixed in `apps/agent/src/runtime.py` by registering a `google_genai` harness that excludes `SummarizationMiddleware`. Restart the agent. Workaround: `AGENT_RUNTIME=gemini-flash-react` (no deepagents stack). |
